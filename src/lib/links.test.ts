...
describe("withRef", () => {
  it("appends utm_source + utm_medium to a bare URL", () => {
    const u = new URL(withRef("https://acme.example"));
  ...
  it("appends utm_campaign when provided", () => {
    const u = new URL(withRef("https://acme.example", "header-nav"));
  ...
  it("preserves existing UTM keys (no clobber)", () => {
    const inUrl =
      "https://acme.example/?utm_source=keep-me&utm_campaign=hand-tagged";
  ...
  it("preserves non-UTM query params", () => {
    const inUrl = "https://acme.example/path?foo=bar&baz=qux";
  ...
  it("preserves the URL path + fragment", () => {
    const out = withRef("https://acme.example/path/to/page#section");
  ...
  it("passes through invalid URLs unchanged", () => {
    ...
    expect(withRef("mailto:hi@acme.example")).toBeTruthy();
  });
